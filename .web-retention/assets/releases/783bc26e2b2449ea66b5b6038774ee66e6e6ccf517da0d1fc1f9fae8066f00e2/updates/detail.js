...
import { assetUrl, assertPageActive, retryButton } from '../js/release-assets.js';
...
function loadScript(path, globalName) {
  ...
  if (!dependencyRequests.has(url)) {
    dependencyRequests.set(url, withTimeout(signal => new Promise((resolve, reject) => {
    ...
    }), { signal: globalThis.window?.__portfolioRelease?.signal }).catch((error) => {
...
async function loadFooter() {
  ...
  try {
    const [markup, site] = await Promise.all([
      fetchResource(assetUrl('../sections/footer/footer.html'), { format: 'text' }),
...
// The Editor's unbundled renderer keeps its existing initialization contract.
if (!window.__portfolioRelease) await initializeDetailPage();

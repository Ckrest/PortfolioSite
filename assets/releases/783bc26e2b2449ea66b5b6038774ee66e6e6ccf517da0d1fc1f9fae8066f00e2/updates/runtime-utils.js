...
import { fetchResource } from '../js/request.js';
...
export function resolveUpdateAsset(value, update) {
  ...
  return assetUrl(`../${documentCollection(update)}/${folder}/${encodedPath(path)}${version ? `${path.includes('?') ? '&' : '?'}v=${encodeURIComponent(version)}` : ''}`);
}
...
export async function fetchUpdateText(value, update) {
  ...
  return fetchResource(url, { format: 'text' });
}
